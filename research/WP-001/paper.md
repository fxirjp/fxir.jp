...
| **直前速度** | 初回介入日の前5営業日の終値5本から算出した**差分4本**の平均絶対値（円/日） |
| **半減期** | 最終介入日の翌営業日以降、**終値**が「最安値 + Impact × 0.5」を初めて回復するまでの営業日数 |

本稿の測定・判定の詳細は方法論 METHOD-v1.0（v1.3で遡及明記）に従う。
...
**説明B（無秩序への反応）**：2026年7月30日の介入直前5時間の1時間足値幅は6.8〜37.0 pips、1分足平均レンジ1.8 pips、最大14.3 pips。標本中で最も静穏な局面である。なお静穏性の測定値は窓の取り方に依存し、監査時の別窓実装では平均6.8／9.2pips相当の値も得られている（窓依存性の明示・R1対応）。少なくとも「価格が暴れているのを見て反応した」という記述は、この事例に当てはまらない。
...
| Ep | 期間 | 期間営業日数 | Cost | Impact | 効率 | 直前速度 | 半減期 |
|---|---|---|---|---|---|---|---|
...
※ Cost・Impact・効率は表示上、小数第2位に丸める。50%回復線の算出は**未丸めのImpact**による（METHOD-v1.0 §七。v1.3追記・R1対応）。
...
**運用細則（PR-001と同一定義・v1.2で本文に明記）**: 生存窓は、最終介入日の翌営業日から半減期到達日の**前日**までとする（到達日当日の事象は「効果の生存中に発生した」と認めない）。営業日は土日を除く週日とし、国内祝日は価格系列が存在するため算入する。

| Ep | 半減期 | ①日銀利上げ | ②DGS2 −50bp | ③RV20>P90 | 判定 |
|---|---|---|---|---|---|
| E1 | 1営業日 | 判定不能（窓なし） | 窓なし | 窓なし | **判定不能** |
| E2 | 5営業日 | 不成立 | 不発（窓内最小4.30／線4.00） | 不発 | +α無 |
| E3 | 15営業日 | 不成立 | 不発（窓内最小4.73／線4.46） | **発火 2024-05-02** ※ | **+α有（③のみ）** |
| E4 | 431営業日 | 発火 +13営業日 | **発火 2024-08-02（3.88）** | **発火 2024-08-05** | **+α有（①②③）** |
| E5 | 5営業日 | 順序不成立 | 不発（窓内最小3.90／線3.37） | 不発 | +α無 |
...
- **エピソードの区切りに客観的基準がない**。何営業日空ければ別エピソードとするかは筆者の判断（一般則はMETHOD v1.1で形式化予定）

### 7.3 未検証・未確定

- ~~+α定義の基準②③が未検証~~ → **v1.1（§4.6）で追試済み**。ただしE1は生存窓が空集合であり全基準が判定不能（§4.6表もv1.2で「判定不能」に統一）、基準③には介入自身の変動による交絡（§4.6※）が残る。半減期が1営業日級のエピソードでは生存窓が構造的に空集合となるため、+α基準は原理的に適用できない
...
- v1.0（2026-08-08）: 公開版
- **v1.1（2026-08-08）**: ①§2.1の日足データ期間を実データ実体（2002年5月〜）に訂正（正本と検証情報の不整合をAuditor v0.2導入の契機として記録）②§4.6を新設し基準②③の追試結果を追記 ③§7.3を更新 ④§4.5を時点明示表現に変更（v1.1確定作業中の修正。§4.6との時間的矛盾の解消）。v1.0正本は `wp-001_v1.0.md` として保存
- **v1.2（2026-08-08T15:40+09:00）**: 外部監査 AUDIT-2026-08-08 への対応。①生存窓・営業日の定義を§4.6に明記し、E1の+α判定を「判定不能」に統一（指摘H-1・L-2）②§3.2の列名を「期間営業日数」に訂正（M-1）③初期草稿の位置づけを「公開前改訂」に整理（C-3）④§4.6の半減期表記を営業日に統一（L-1）。v1.1正本は `wp-001_v1.1.md` として保存
- **v1.3（2026-08-08T22:37+09:00）**: 外部監査R1の残指摘への対応。①§2.3の直前速度を一意定義化（終値5本→差分4本）②§3.1の静穏性測定に窓依存の注記③§3.2にImpact表示丸めと閾値の未丸め算出の注記④METHOD-v1.0への準拠を明記⑤§7.2にグルーピング形式化予定を注記。v1.2正本は `paper_v1.2.md`（配備名 `wp-001_v1.2.md`）として保存
